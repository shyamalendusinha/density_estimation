...
zdfunction = function(q) sample(kd, q, prob=pid, replace=TRUE)

mud = sigma2d =vector()

r.biv.density = function(q,zd){
  for (i in 1:q){
...
d.biv.density = function(x, y){
  ans = rep(0,length(x))
  for (r in 1:kd){
    ans = ans + pid[r]*dnorm(x, mean=meand[r], sd=sqrt(y/lambdad[r]))*
      dinvgamma(y, shape=sigmashape[r], rate=sigmarate[r])
  }
  return(ans)
}

#marginal densities not correct one
d.mu.marginal.density = function(x) {
  ans = rep(0,length(x))
  for (r in 1:kd){
    ans = ans + pid[r]*sqrt(lambdad[r]*sigmashape[r]/sigmarate[r])*
      dt(sqrt(lambdad[r]*sigmashape[r]/sigmarate[r])*(x-meand[r]), df=2*sigmashape[r])
  }
  return(ans)
}

d.sigma2.marginal.density = function(y){
  ans = rep(0,length(y))
  for (r in 1:kd){
    ans = ans + pid[r]*dinvgamma(y, shape=sigmashape[r], rate=sigmarate[r])
  }
  return(ans)
}
...
alloutput_diffq = foreach(b = 1:B,.combine='cbind',.inorder=FALSE) %dopar% {
  ...
  zd = zdfunction(q)
  random.sample = r.biv.density(q,zd)
