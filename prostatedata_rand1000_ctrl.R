...
outputfile = "prostate_1000ctrl.Rdata"
outputfilecsv = "prostate_1000ctrl.csv"
...
rows = sample(dim(data)[1],1000)
cols = sample(dim(data)[2],4)
...
# true densities on the grid
  ...
  MCMCoutput = DPMM.nig(x.vec, ids, gamma.pi=0.1, 
                        k=10, Burnin=3000, Simsize=5000, mu.grid=mu.grid, sigma2.grid=sigma2.grid, 
  ...
  MCMCoutput_abhra = UNIVARIATE_DECON_HETEROSCEDASTIC(ws=x.vec, mis=ni.vec, error.fit=c("normal"), x.grid=mu.grid,
                                                      simsize=5000, burnin=3000, plot_results=FALSE)
