...
r.biv.density = function(q){
  ...
  mud = rexp(q, rate = 1/2)
  for(i in 1:q){
    sigma2d[i] = rchisq(1, df = mud[i])+0.1
  }
  return(list(mud = mud, sigma2d = sigma2d))
}

d.biv.density = function(x,y){
  ans = dexp(x, rate = 1/2) * dchisq(y-0.1, df = x)
  return(ans)
}
...
d.sigma2.density = function(y){
  ans = density(c(y,0.2-y), n=n2, from=sigma2.grid[1], to=sigma2.grid[n2])$y
  return(ans)
}
...
alloutput_diffq = foreach(b = 1:B,.combine='cbind',.inorder=FALSE) %dopar% {
  ...
  d.sigma2.true = density(c(sigma2long,0.2-sigma2long), n=n2, from=sigma2.grid[1], 
                          to=sigma2.grid[n2])$y
  
  # KDE on true parameters
  d.biv.true.kde =c(kde2d(c(mud,mud,-mud,-mud),c(sigma2d,0.2-sigma2d,sigma2d,0.2-sigma2d),n=c(n1,n2),lims=c(range(mu.grid),
                                                                           range(sigma2.grid)))$z)
