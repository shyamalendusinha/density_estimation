...
r.biv.density = function(q,zd){
  for (i in 1:q){
    sigma2d[i] = runif(1, min=sigma2min[zd[i]], max=sigma2max[zd[i]])
    mud[i] = runif(1, min=meanmin[zd[i]], max=meanmax[zd[i]])
  }
...
d.biv.density = function(x, y){
  ans = rep(0,length(x))
  for (r in 1:kd){
    ans = ans + pid[r]*dunif(x, min=meanmin[r], max=meanmax[r])*
      dunif(y, min=sigma2min[r], max=sigma2max[r])
  }
  return(ans)
}
...
d.sigma2.marginal.density = function(y){
  ans = rep(0,length(y))
  for (r in 1:kd){
    ans = ans + pid[r]*dunif(y, min=sigma2min[r], max=sigma2max[r])
  }
  return(ans)
}
