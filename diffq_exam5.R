...
r.biv.density = function(q){
  mud = sigma2d = rep(0,q)
  mud = rinvchisq(q, df = mu_df)
  for(i in 1:q){
    sigma2d[i] = rlnorm(1, meanlog = log(mud[i]), sdlog = log_sd)
  }
  return(list(mud = mud, sigma2d = sigma2d))
}

d.biv.density = function(x,y){
  ans = dinvchisq(x, df = mu_df) * 
    dlnorm(y, meanlog = log(x), sd = log_sd)
  return(ans)
}

d.mu.density = function(x){
  n1 = length(x)
  ans = dinvchisq(x, df = mu_df)
  return(ans)
}

d.sigma2.density = function(y){
  n1 = length(x)
  ans = density(c(y,-y), n=n2, from=sigma2.grid[1], to=sigma2.grid[n2])$y
  return(ans)
}
...
alloutput_diffq = foreach(b = 1:B,.combine='cbind',.inorder=FALSE) %dopar% {
  ...
  mu.grid = seq(max(min(xbar.vec)-IQR(xbar.vec),0.001), max(xbar.vec)+IQR(xbar.vec), 
                length.out=2*n1+1)[seq(2,2*n1,2)]
  ...
  d.mu.true = d.mu.density(mu.grid)
  
  sigma2long = r.biv.density(100000)$sigma2d
  d.sigma2.true = density(c(sigma2long,-sigma2long), n=n2, from=sigma2.grid[1], 
                          to=sigma2.grid[n2])$y
  
  # KDE on true parameters
  d.biv.true.kde =c(kde2d(c(mud,mud,-mud,-mud),c(sigma2d,-sigma2d,sigma2d,-sigma2d),n=c(n1,n2),lims=c(range(mu.grid),
                                                                           range(sigma2.grid)))$z)
  d.mu.true.kde = density(c(mud,-mud), n=n1, from=mu.grid[1], to=mu.grid[n1])$y
  ...
  MCMCoutput = DPMM.nig(x.vec, ids, gamma.pi=0.1, 
                        k=30, Burnin=5000, Simsize=5000, mu.grid=mu.grid, sigma2.grid=sigma2.grid, 
  ...
  MCMCoutput_ng = DPMM.ng(x.vec, ids, gamma.pi=0.1, 
                          k=30, Burnin=5000, Simsize=5000, mu.grid=mu.grid, prec.grid=prec.grid, 
                          hyperparameters = c('Default'))
